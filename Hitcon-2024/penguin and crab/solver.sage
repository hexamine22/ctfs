...
flag = [0] * 25
flag[12] = 0xbee66f8f #brute forced final loop for this val
remainders = []
...
decrypted_flag = ""
for i in range(6):
    temp = remainders[counter] + (div_cmp_arr[counter] * flag[12])
...
for i in range(6):
    ...
    decrypted_first_value = decrypt_value(first_val,dec_idx)
    decrypted_second_value = decrypt_value(second_val,dec_idx-1)
    part2 += hex(decrypted_first_value)[2:] + hex(decrypted_second_value)[2:]
    counter += 1
    dec_idx -=2
    
part2 = bytearray.fromhex(part2).decode()
decrypted_flag = bytearray.fromhex(decrypted_flag).decode()

i = len(part2)
part2 = part2[::-1]
for i in range(0,len(part2),4):
    decrypted_flag += part2[i:i+4][::-1]
